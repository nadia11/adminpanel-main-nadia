...
class DriverTripController extends Controller
{
    public function driver_trips()
    {
        $driver_trip_info = DB::table('driver_trips')
            ->leftjoin('drivers', 'driver_trips.driver_id', '=', 'drivers.driver_id')
            ->select('driver_trips.*', 'drivers.driver_name', 'drivers.mobile as driver_mobile')
            ->orderBy('created_at', 'DESC')->get();

        $manage_driver_trip = view('pages.drivers.driver-trips')->with('all_driver_trip_info', $driver_trip_info);

        return view('dashboard')->with('main_content', $manage_driver_trip);
    }
    ...
    public function driver_trip_route($trip_id){
        $driver_trip_info = DB::table('driver_trips')
            ->leftjoin('drivers', 'driver_trips.driver_id', '=', 'drivers.driver_id')
            ->select('driver_trips.*', 'drivers.driver_name', 'drivers.mobile as driver_mobile')
            ->where('trip_status', 'booked')->orderBy('created_at', 'DESC')->get();

        $manage_driver_trip = view('pages.drivers.driver-trip-route')->with('all_driver_trip_info', $driver_trip_info);

        return view('dashboard')->with('main_content', $manage_driver_trip);
    }

    public function new_driver_trip_save00( Request $request)
    {
//        dd($request->all());
...
//        ]);
        ...
        DB::table('driver_trips')->insert( $data );

        return redirect('/trip/trip-management')->with('success', 'Created New Purchase Successfully!');
    }

    public function delete_driver_trip00( Request $request, $trip_id)
    {
        if ( $request->ajax() ) {
            DB::table('driver_trips')->where('trip_id', $trip_id)->delete();
